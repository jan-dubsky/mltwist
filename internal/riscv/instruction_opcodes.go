...
func opcodeShiftImm(arithmetic bool, shiftBits uint8, mid byte, low byte) opcode.Opcode {
	...
	// to get 2^shiftBits. The we subtract 1 which creates is a bit mask for
	// bits encoding values 0..(2^shiftBits)-1. We then invert the mask to
	// force all reserved bits to be zero.
	//
