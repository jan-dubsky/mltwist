...
func valUint(v uint64, w expr.Width) expreval.Value {
	return expreval.ParseConst(expr.NewConstUint(v, w))
}

func newValue(bs []byte) expreval.Value {
	if ln := len(bs); int(expr.Width(ln)) != ln {
		panic(fmt.Sprintf("byte array is too long: %d", ln))
	}

	return expreval.ParseConst(expr.NewConst(bs, expr.Width(len(bs))))
}
...
func TestAdd(t *testing.T) {
	tests := []binaryOpTest{{
		...
		name:   "sum_with_carry",
		v1:     newValue([]byte{255}),
		v2:     newValue([]byte{1}),
		w:      expr.Width32,
		result: newValue([]byte{0, 1, 0, 0}),
	}, {
		name:   "carry_chain",
		v1:     newValue([]byte{255, 255, 255, 255, 255}),
		v2:     newValue([]byte{7, 67, 23, 43}),
		w:      expr.Width64,
		result: newValue([]byte{6, 67, 23, 43, 0, 1, 0, 0}),
	}, {
		name:   "cut_upper_bytes",
		v1:     newValue([]byte{5, 6, 7, 8}),
		v2:     newValue([]byte{34, 255}),
		w:      expr.Width16,
		result: newValue([]byte{39, 5}),
	}}
...
func TestLsh(t *testing.T) {
	tests := []binaryOpTest{{
		name:   "byte_shift",
		v1:     newValue([]byte{1, 2, 3, 4}),
		v2:     valUint(16, expr.Width8),
		w:      expr.Width32,
		result: newValue([]byte{0, 0, 1, 2}),
	}, {
		name:   "bit_shift",
		v1:     newValue([]byte{0xff, 0, 0x0f, 0x2}),
		v2:     valUint(3, expr.Width8),
		w:      expr.Width32,
		result: newValue([]byte{0xf8, 7, 0x78, 0x10}),
	}, {
		name:   "shift_cut_to_zero",
		v1:     valUint(math.MaxUint64, expr.Width64),
		v2:     newValue([]byte{0, 0, 0, 0, 0, 0, 0, 0, 1}),
		w:      expr.Width64,
...
func TestRsh(t *testing.T) {
	tests := []binaryOpTest{{
		...
		v2:     valUint(16, expr.Width8),
		w:      expr.Width32,
		result: newValue([]byte{3, 4, 0, 0}),
	}, {
		name:   "bit_shift",
		v1:     newValue([]byte{0xff, 0, 0x0f, 0x2}),
		v2:     valUint(3, expr.Width8),
		w:      expr.Width32,
		result: newValue([]byte{0x1f, 0xe0, 0x41, 0}),
	}, {
		name:   "shift_cut_to_zero",
		v1:     valUint(math.MaxUint64, expr.Width64),
		v2:     newValue([]byte{0, 0, 0, 0, 0, 0, 0, 0, 1}),
...
func TestMul(t *testing.T) {
	tests := []binaryOpTest{{
		name:   "single_byte",
		v1:     newValue([]byte{255}),
		v2:     newValue([]byte{255}),
		w:      2,
		result: newValue([]byte{0x01, 0xfe}),
	}, {
		name:   "two_bytes",
		...
		name:   "cut_upper_bytes",
		v1:     newValue([]byte{0, 0, 5, 6}),
		v2:     newValue([]byte{45, 67}),
		w:      expr.Width16,
		result: newValue([]byte{0, 0}),
	}, {
		name:   "overflow",
		v1:     newValue([]byte{0, 0, 0xff, 0x1}),
		v2:     newValue([]byte{0x5, 0x7, 0x23, 0x71}),
		w:      expr.Width32,
		result: newValue([]byte{0x0, 0x0, 0xfb, 0x2}),
	}}
...
func TestDiv(t *testing.T) {
	tests := []struct {
		...
		{
			name: "single_byte",
			v1:   newValue([]byte{183}),
			v2:   newValue([]byte{23}),
			w:    expr.Width8,
			div:  newValue([]byte{183 / 23}),
		},
		{
			name: "two_bytes",
		...
		},
		{
			...
			v2:   newValue([]byte{0}),
			w:    expr.Width32,
			div:  newValue([]byte{0xff, 0xff, 0xff, 0xff}),
		},
	}
...
func TestNand(t *testing.T) {
	tests := []struct {
		...
		{
			name: "multi_byte",
			v1:   newValue([]byte{0xfb, 0x45, 0x43, 0x2b}),
			v2:   newValue([]byte{0xde, 0x04, 0x22, 0x9f}),
			w:    expr.Width32,
			exp:  newValue([]byte{0x25, 0xfb, 0xfd, 0xf4}),
		},
		{
			name: "cut_and_extend",
			v1:   newValue([]byte{0x55, 0x23, 0xb2, 0x3a, 0x24, 0x6f, 0x34, 0xbb}),
			v2:   newValue([]byte{0x56, 0xb3}),
			w:    expr.Width32,
			exp:  newValue([]byte{0xab, 0xdc, 0xff, 0xff}),
		},
	}
